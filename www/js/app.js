// Generated by Haxe 4.0.0-rc.5+6174eb9a9
...
App.main = function() {
	...
	var pre = (_this1["iterator"])();
	while(pre.hasNext()) {
		var pre1 = pre.next();
		var div = js.JQuery("<div>").addClass("pre");
		var _g2 = 0;
		var _g11 = pre1.html().split("\n");
	...
	var changeSub = function() {
		...
		if(p1.hasClass("highlight")) {
			var p2 = (parts["iterator"])();
			while(p2.hasNext()) {
				var p21 = p2.next();
				p21.toggleClass("light",p1[0] == p21[0]).toggleClass("unlight",p1[0] != p21[0]);
			}
		}
		sub += 1;
		update();
	...
	var s2 = (_this2["iterator"])();
	while(s2.hasNext()) {
		var s3 = s2.next();
		var p3 = s3.wrap("<div class='slide-container'>").parent();
		s3.prepend(js.JQuery("<div>").addClass("slide-bg"));
		var id = [slides.length];
		var parts1 = clickThrough ? "*" : "empty";
		var parts2 = [s3.find("li,pre,h2,p,div.pre .line,.click").not("li > pre:first-child").filter(parts1)];
		var tmp = parts2[0].not(".visible");
		parts2[0] = tmp;
		parts2[0].hide();
		if(id[0] == cur) {
			var _g3 = 0;
			...
			while(_g3 < _g12) {
				var i1 = _g3++;
				js.JQuery(parts2[0][i1]).show().parent().show();
			}
		}
		slides.push({ slide : p3, parts : parts2[0], sub : 0});
		p3.hide();
		var forward = [(function(parts3,id1) {
			return function() {
				if(sub < parts3[0].length) {
					changeSub();
				} else {
					changeSlide(id1[0] + 1);
				}
	...
	var ol = js.JQuery("<ol>");
	var _g4 = 0;
	var _g13 = slides.length;
	while(_g4 < _g13) {
		var i2 = _g4++;
		var title = slides[i2].slide.find("h1").eq(0).text();
	...
	var onResize = function() {
		...
		var s4 = Math.min(j.width() / 800,j.height() / 600);
		js.JQuery("body.fullScreen .slide").css({ "-webkit-transform" : "scale(" + s4 + ")"});
	};
	...
	var h = (_this3["iterator"])();
	while(h.hasNext()) {
		var h1 = h.next();
		var count = [0];
		if(h1.text().indexOf(" ") < 0) {
			continue;
		}
		h1.html(new EReg("(@|[^ ]+)","g").map(h1.text(),(function(count1) {
			return function(r1) {
